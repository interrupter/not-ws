...
class notWSConnection extends EventEmitter {
    ...
    scheduleReconnect() {
        if (!this.slave) {
            ...
            this.connectInterval = setInterval(
                this.performReconnect.bind(this),
                timeout
            );
        }
    }
        ...
        if (this.slave) {
            ...
            if (this.options.ping) {
                ...
                this.pingInterval = setInterval(
                    ...
                    this.options.pingTimeout || CONST.PING_TIMEOUT
                );
            }
        }
    }
    ...
    async send(data, secure) {
        ...
        try {
            if (
                this.isConnected(secure) ||
                (this.isOpen() && this.isMessageTokenUpdateRequest(data))
            ) {
        ...
        if (Object.values(CONST.STATE).indexOf(state) > -1) {
            ...
            switch (this[SYMBOL_STATE]) {
                ...
                case CONST.STATE.NOT_CONNECTED:
                    if (
                        [CONST.STATE.CONNECTED, CONST.STATE.ERRORED].indexOf(
                            state
                        ) > -1
                    ) {
                        ...
                        throw new CONST.notWSException(
                            "Wrong state transition: " +
                                CONST.STATE_NAME[this[SYMBOL_STATE]] +
                                " -> " +
            ...
            throw new CONST.notWSException(
                "set: Unknown notWSServerClient state: " + state
            );
        }
    }
...
*/
    set activity(activity = CONST.ACTIVITY.IDLE) {
        if (Object.values(CONST.ACTIVITY).indexOf(activity) > -1) {
            ...
            throw new CONST.notWSException(
                "set: Unknown notWSServerClient activity: " + activity
            );
        }
    }
    ...
    countPassed(input, where) {
        this.passed[where] += this.options.count
            ? this.getMessageSize(input)
            : 0;
        return input;
    }
