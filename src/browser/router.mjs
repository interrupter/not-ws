	...
	async route({type, name, cred}, data, client){
      ...
      Func.ObjHas(this.routes[type], name)
			...
			if(Array.isArray(this.routes[type][name]) && this.routes[type][name].length > 1){
				...
				await Promise.all(
					guards.map((guard) => Func.executeFunctionAsAsync(guard, [{data, cred, client}]))
				);
				return Func.executeFunctionAsAsync(actualRoute,[{data, cred, client}]);

			}else if(Func.isFunc(this.routes[type][name])){
				return await Func.executeFunctionAsAsync(this.routes[type][name], [{data, cred, client}]);
			}
		}
