...
class notWSRouter extends EventEmitter {
    ...
    async route({ type, name, cred }, data, client) {
        const identity = notAppIdentity.extractAuthData(client.identity);
        if (
            ...
            if (
                Array.isArray(this.routes[type][name]) &&
                this.routes[type][name].length > 1
            ) {
                ...
                await Promise.all(
                    guards.map((guard) =>
                        Func.executeFunctionAsAsync(guard, [
                            { data, cred, client, identity },
                        ])
                    )
                );
                ...
                return await Func.executeFunctionAsAsync(
