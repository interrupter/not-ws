const uuid = require("uuid"),
    ...
    { notWSException } = require("../src/node/const.js");
...
describe("notWSMessage", () => {
    ...
    it("getErrorMessage from error object", () => {
        ...
        expect(msg.getErrorMessage(message)).to.be.deep.equal(
            "200: legacy error"
        );
    });
    ...
    it("pack message with error, without cred", () => {
        let msg = new notWSMessage(),
            ...
            error = new notWSException("error");
        error.code = 1;
        let serviceData = {
                type: "response",
                name: "auth",
    ...
    it("validateTypeAndName: invalid type, invalid name, insecure", () => {
        ...
        expect(msg.validateTypeAndName(message.type, message.name)).to.be.not
            .ok;
    });
