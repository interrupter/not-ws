...
describe("notWSRouter", () => {
    ...
    it("route: route with guards", (done) => {
        let guardsRunned = 0;
        let routes = {
                method: {
                    man: [
                        ...
                        //actual route
                        () => {
                            expect(guardsRunned).to.be.equal(2);
                            done();
                        },
                    ],
                },
        ...
        router
            .route(
                { type: "method", name: "man", cred: null },
                {},
                {
                    ip: "127.0.0.1",
                    getIP() {
    ...
    it("route: route with guards, one guard throws exception", (done) => {
        let guardsRunned = 0;
        let routes = {
                method: {
                    man: [
                        ...
                        async () => {
                            guardsRunned++;
                            throw new Error("guard test error");
                        },
                        //actual route
                        () => {
                            done(
                                new Error(
                                    "route shouldnt be runned, because guards throwed"
                                )
                            );
                        },
                    ],
        ...
        router
            ...
            .catch((e) => {
                expect(e).to.be.instanceof(Error);
                expect(e.message).to.be.equal("guard test error");
                done();
            });
    });
    ...
    it("setRoutesForType: type is invalid: null", (done) => {
        ...
        try {
            ...
            expect(e.message).to.be.equal(
                "Route's type name should be a String!"
            );
            done();
        }
    });
    ...
    it("setRoutesForType: good type, routes is invalid: null", (done) => {
        ...
        try {
            ...
            expect(e).to.be.instanceof(Error);
            expect(e.message).to.be.equal(
                "Route's type's routes set should be an Object!"
            );
            done();
        }
    });
    ...
    it("unsetRoutesForType: good type - exists, bad list of routes: null", (done) => {
        ...
        try {
            ...
            expect(e).to.be.instanceof(Error);
            expect(e.message).to.be.equal(
                "List of routes names should be an Array!"
            );
            done();
        }
    });
    ...
    it("unsetRoutesForType: good type - exists, list is not specified", () => {
        ...
        expect(Object.keys(router.routes).length).to.be.equal(3);
        expect(router.routes.test).to.have.keys([
            "kid",
            "sayHello",
            "woman",
            "man",
